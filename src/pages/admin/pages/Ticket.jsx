...
function Card({ ticket, onClick }) {
  return (
    <div onClick={onClick} className="bg-white rounded-xl shadow-sm border p-4 hover:shadow-md transition-shadow cursor-pointer">
      <div className="mb-3">
  ...
  return (
      <main className="max-w-5xl mx-auto px-3 sm:px-4 lg:px-6 py-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {paged.map((ticket) => (
            <Card key={ticket.id} ticket={ticket} onClick={() => navigate(`/admin/tickets/${ticket.id}`)} />
          ))}
        </div>
